...
typedef struct {
    ...
    esp_http_client_h_transport_t  h_transport_type;
...
struct esp_http_client {
    ...
    h_transport_list_handle_t     h_transport_list;
    h_transport_handle_t          transport;
...
static esp_err_t _set_config(esp_http_client_handle_t client, const esp_http_client_config_t *config)
{
    ...
    if (config->h_transport_type == HTTP_TRANSPORT_OVER_SSL) {
...
esp_http_client_handle_t esp_http_client_init(const esp_http_client_config_t *config)
{
    ...
    _success = (
                   (client->h_transport_list = h_transport_list_init()) &&
                   (tcp = h_transport_tcp_init()) &&
                   (h_transport_set_default_port(tcp, DEFAULT_HTTP_PORT) == ESP_OK) &&
                   (h_transport_list_add(client->h_transport_list, tcp, "http") == ESP_OK)
               );
    if (!_success) {
...
#ifdef CONFIG_ESP_HTTP_CLIENT_ENABLE_HTTPS
    h_transport_handle_t ssl;
    _success = (
                   (ssl = h_transport_ssl_init()) &&
                   (h_transport_set_default_port(ssl, DEFAULT_HTTPS_PORT) == ESP_OK) &&
                   (h_transport_list_add(client->h_transport_list, ssl, "https") == ESP_OK)
               );
    ...
    if (config->cert_pem) {
        h_transport_ssl_set_cert_data(ssl, config->cert_pem, strlen(config->cert_pem));
    }
#endif
...
esp_err_t esp_http_client_cleanup(esp_http_client_handle_t client)
{
    ...
    h_transport_list_destroy(client->h_transport_list);
...
static int esp_http_client_get_data(esp_http_client_handle_t client)
{
    ...
    int rlen = h_transport_read(client->transport, res_buffer->data, client->buffer_size, client->timeout_ms);
    if (rlen >= 0) {
...
int esp_http_client_read(esp_http_client_handle_t client, char *buffer, int len)
{
    ...
    while (need_read > 0 && is_data_remain) {
        ...
        rlen = h_transport_read(client->transport, res_buffer->data, byte_to_read, client->timeout_ms);
...
int esp_http_client_fetch_headers(esp_http_client_handle_t client)
{
    ...
    while (client->state < HTTP_STATE_RES_COMPLETE_HEADER) {
        buffer->len = h_transport_read(client->transport, buffer->data, client->buffer_size, client->timeout_ms);
        if (buffer->len <= 0) {
            return ESP_FAIL;
        }
...
esp_err_t esp_http_client_open(esp_http_client_handle_t client, int write_len)
{
    ...
    if (client->state < HTTP_STATE_CONNECTED) {
        ...
        client->transport = h_transport_list_get_transport(client->h_transport_list, client->connection_info.scheme);
        if (client->transport == NULL) {
            ESP_LOGE(TAG, "No transport found");
            return ESP_ERR_HTTP_INVALID_TRANSPORT;
        }
        if (h_transport_connect(client->transport, client->connection_info.host, client->connection_info.port, client->timeout_ms) < 0) {
    ...
    while ((header_index = http_header_generate_string(client->request->headers, header_index, client->request->buffer->data + first_line, &wlen))) {
        ...
        if (h_transport_write(client->transport, client->request->buffer->data, wlen, client->timeout_ms) <= 0) {
...
int esp_http_client_write(esp_http_client_handle_t client, const char *buffer, int len)
{
    ...
    while (len > 0) {
        ...
        wlen = h_transport_write(client->transport, buffer + widx, need_write, client->timeout_ms);
        if (wlen <= 0) {
            return wlen;
        }
        widx += wlen;
...
esp_err_t esp_http_client_close(esp_http_client_handle_t client)
{
    if (client->state >= HTTP_STATE_INIT) {
        ...
        client->state = HTTP_STATE_INIT;
        return h_transport_close(client->transport);
    }
    return ESP_OK;
}
...
esp_http_client_h_transport_t esp_http_client_get_h_transport_type(esp_http_client_handle_t client)
{
