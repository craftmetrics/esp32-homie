...
static int ssl_close(h_transport_handle_t t);

static int ssl_connect(h_transport_handle_t t, const char *host, int port, int timeout_ms)
{
    int ret = -1, flags;
    struct timeval tv;
    h_transport_ssl_t *ssl = h_transport_get_context_data(t);
...
static int ssl_poll_read(h_transport_handle_t t, int timeout_ms)
{
    h_transport_ssl_t *ssl = h_transport_get_context_data(t);
    fd_set readset;
    FD_ZERO(&readset);
...
static int ssl_poll_write(h_transport_handle_t t, int timeout_ms)
{
    h_transport_ssl_t *ssl = h_transport_get_context_data(t);
    fd_set writeset;
    FD_ZERO(&writeset);
...
static int ssl_write(h_transport_handle_t t, const char *buffer, int len, int timeout_ms)
{
    int poll, ret;
    h_transport_ssl_t *ssl = h_transport_get_context_data(t);

    if ((poll = h_transport_poll_write(t, timeout_ms)) <= 0) {
...
static int ssl_read(h_transport_handle_t t, char *buffer, int len, int timeout_ms)
{
    int poll = -1, ret;
    h_transport_ssl_t *ssl = h_transport_get_context_data(t);
    if (mbedtls_ssl_get_bytes_avail(&ssl->ctx) <= 0) {
        if ((poll = h_transport_poll_read(t, timeout_ms)) <= 0) {
            return poll;
        }
    }
...
static int ssl_destroy(h_transport_handle_t t)
{
    h_transport_ssl_t *ssl = h_transport_get_context_data(t);
    h_transport_close(t);
    free(ssl);
...
void h_transport_ssl_set_cert_data(h_transport_handle_t t, const char *data, int len)
{
    h_transport_ssl_t *ssl = h_transport_get_context_data(t);
    if (t && ssl) {
...
h_transport_handle_t h_transport_ssl_init()
{
    h_transport_handle_t t = h_transport_init();
    h_transport_ssl_t *ssl = calloc(1, sizeof(h_transport_ssl_t));
    HTTP_MEM_CHECK(TAG, ssl, return NULL);
    mbedtls_net_init(&ssl->client_fd);
    h_transport_set_context_data(t, ssl);
    h_transport_set_func(t, ssl_connect, ssl_read, ssl_write, ssl_close, ssl_poll_read, ssl_poll_write, ssl_destroy);
    return t;
}
